...
export class QualityInspectionService {
  ...
  async finishInspection(
    ...
    if (data.inspected_qty > batch.produced_qty!) {
      throw new Error(`Inspected quantity (${data.inspected_qty}) cannot exceed produced quantity (${batch.produced_qty})`);
    }
    ...
    if (data.inspection_mode === 'full' && data.inspected_qty !== batch.produced_qty) {
      throw new Error('For full inspection, inspected quantity must equal produced quantity');
    }
